                            ...
                            withCredentials([usernamePassword(credentialsId: 'harbor-creds', usernameVariable: 'HARBOR_USER', passwordVariable: 'HARBOR_PASS')]) {
                                script {
                                    try {
                                        ...
                                        echo "Harbor login failed, retrying..."
                                        sleep 5
                                        sh "echo \$HARBOR_PASS | docker login ${HARBOR_URL} -u \$HARBOR_USER --password-stdin"
                                    }
                                }
                ...
                steps {
                    script {
                        ...
                        if(params.ACTION in ['FULL_PIPELINE','FRONTEND']) {
                            sh "kubectl set image deployment/frontend frontend=${HARBOR_URL}/${HARBOR_PROJECT}/frontend:${IMAGE_TAG} || kubectl apply -f k8s/frontend-deployment.yaml"
                        }

                        // Backend
                        if(params.ACTION in ['FULL_PIPELINE','BACKEND']) {
                            sh "kubectl set image deployment/backend backend=${HARBOR_URL}/${HARBOR_PROJECT}/backend:${IMAGE_TAG} || kubectl apply -f k8s/backend-deployment.yaml"
                        }
